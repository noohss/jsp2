...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%
	String path = "/Users/parksanghoon/Desktop/psh_std/upload";
								// 서버의 업로드 경로에 대해 url 지정이 필요하다. server.xml
	...
	// DefaultFileRenamePolicy 객체 : 업로드한 파일명이 중복 될 경우 파일명을 변경하는 정책을 설정.
	//				ㄴ 해쉬값(랜덤문자열) 이용하거나 업로드 날짜 시간을 이용해서 파일명을 변경할 수 있다.
	...
	// 파일 자체는 서버의 업로드 폴더에서 보관, 파일명은 데이터베이스 테이블에 저장
	Gallery vo = new Gallery(0,title,null,filename);
	GalleryDao dao = GalleryDao.getInstance();
	dao.insert(vo);
	response.sendRedirect("galleryAction.jsp");	// 업로드한 이미지
%>