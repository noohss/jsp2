...
<ul>
	...
	<li>jstl 라이브러리 필요</li>
	<li>el은 expression language의 약자. 기호는 $&#123;&#125; 사용
	<%-- <%= %> --%> 기호 대신 사용. 애트리뷰트 값 getAttribute 메소드 동작 실행 후 출력</li>
	<li>jstl과 el은 함께 사용된다.(el은 라이브러리 추가 없이 사용한다.)</li>
</ul>
...
<c:choose>
	...
	<c:when test="${age < 23 }">	<!-- 첫번째 조건이 거짓일때만  -->
...
<c:forEach var="x" begin="10" end="30" step="2" varStatus="status">	<!-- varStatus는 현재 상태 저장할 변수 -->
	<c:out value="${x }"/>, index =
	<c:out value="${status.count }"/><br>	<!-- varStatus의 count 속성은 1부터 시작하여 몇번째 값인지 출력 -->
</c:forEach>

<h4>forTokens</h4>
<c:set var="twice" value="사나,나연,다현,모모"/>
<ul>
<c:forTokens items="${twice }" var="member" delims=",">		<!-- delims는 구분 기호 -->
	<li>
